...
class DHCPServer(object):
    ...
    def _getNextDHCPPacket(self, timeout=60, packet_buffer=2048):
        ...
        :return tuple(2): (DHCP-packet-received:``bool``,
                          ...
                          timeout)
        """
        (source_address, data, port) = self._network_link.getData(
            timeout=timeout, packet_buffer=packet_buffer
        )
        if data:
            try:
                ...
                if packet.isDHCPRequestPacket():
                    if (
                        self._handleDHCPRequest.__func__
                        is not DHCPServer._handleDHCPRequest
                    ):  # only spawn a thread if there's an implementation to handle the packet
                        threading.Thread(
                            target=self._handleDHCPRequest,
                            args=(packet, source_address, port),
                        ).start()
                elif packet.isDHCPDiscoverPacket():
                    if (
                        self._handleDHCPDiscover.__func__
                        is not DHCPServer._handleDHCPDiscover
                    ):
                        threading.Thread(
                            target=self._handleDHCPDiscover,
                            args=(packet, source_address, port),
                        ).start()
                elif packet.isDHCPInformPacket():
                    if (
                        self._handleDHCPInform.__func__
                        is not DHCPServer._handleDHCPInform
                    ):
                        threading.Thread(
                            target=self._handleDHCPInform,
                            args=(packet, source_address, port),
                        ).start()
                elif packet.isDHCPReleasePacket():
                    if (
                        self._handleDHCPRelease.__func__
                        is not DHCPServer._handleDHCPRelease
                    ):
                        threading.Thread(
                            target=self._handleDHCPRelease,
                            args=(packet, source_address, port),
                        ).start()
                elif packet.isDHCPDeclinePacket():
                    if (
                        self._handleDHCPDecline.__func__
                        is not DHCPServer._handleDHCPDecline
                    ):
                        threading.Thread(
                            target=self._handleDHCPDecline,
                            args=(packet, source_address, port),
                        ).start()
                elif packet.isDHCPLeaseQueryPacket():
                    if (
                        self._handleDHCPLeaseQuery.__func__
                        is not DHCPServer._handleDHCPLeaseQuery
                    ):
                        threading.Thread(
                            target=self._handleDHCPLeaseQuery,
                            args=(packet, source_address, port),
                        ).start()
...
class _NetworkLink(object):
    ...
    _proxy_port = (
        None  #: The port on which ProxyDHCP traffic is expected to be exchanged.
    )
    _proxy_socket = None  #: The internal socket to use for ProxyDHCP traffic.
    _responder_dhcp = (
        None  #: The internal socket to use for responding to DHCP requests.
    )
    _responder_proxy = (
        None  #: The internal socket to use for responding to ProxyDHCP requests.
    )
    _responder_broadcast = (
        None  #: The internal socket to use for responding to broadcast requests.
    )
    _listening_sockets = None  #: All sockets on which to listen for activity.
    _unicast_discover_supported = (
        False  #: Whether unicast responses to DISCOVERs are supported.
    )
    ...
    def __init__(
        ...
        (dhcp_socket, proxy_socket) = self._setupListeningSockets(
            server_port, proxy_port, server_address
        )
        if proxy_socket:
        ...
        if response_interface:
            try:
                self._responder_broadcast = _L2Responder_AF_PACKET(
                    server_address, response_interface, qtags=response_interface_qtags
                )
            except Exception:
                try:
                    self._responder_broadcast = _L2Responder_pcap(
                        server_address,
                        response_interface,
                        qtags=response_interface_qtags,
                    )
                    ...
                    raise EnvironmentError(
                        errno.ELIBACC,
                        "Raw response-socket requested on {interface}, but neither AF_PACKET nor libpcap are available, or the interface does not exist".format(
    ...
    def _setupListeningSockets(self, server_port, proxy_port, server_address=None):
        ...
        try:
            ...
            warnings.warn(
                "Unable to set SO_REUSEADDR; multiple DHCP servers cannot be run in parallel: {}".format(
                    e
                )
            )
        ...
        if platform.system() != "Linux":
            try:
                ...
                warnings.warn(
                    "Unable to set SO_REUSEPORT; multiple DHCP servers cannot be run in parallel: {}".format(
                        e
                    )
                )
        ...
        if server_address:
            ...
            try:
                dhcp_socket.setsockopt(
                    socket.SOL_SOCKET,
                    _SO_BINDTODEVICE,
                    listen_interface.encode("utf-8"),
                )
            except socket.error as e:
                raise OSError(
                    e.errno,
                    "Unable to limit listening to {}: {}".format(
                        listen_interface, e.strerror
                    ),
                )
            try:
                ...
                raise OSError(
                    e.errno, "Unable to disable routing: {}".format(e.strerror)
                )
    ...
    def sendData(self, packet, address, port):
        ...
        if (
            address.ip in IP_UNSPECIFIED_FILTER
        ):  # Broadcast source; this is never valid for ProxyDHCP
            if (
                not self._unicast_discover_supported  # All responses have to be via broadcast
                or packet.getFlag(FLAGBIT_BROADCAST)
            ):  # Broadcast bit set; respond in kind
                ip = _IP_BROADCAST
            ...
            if relayed:  # Relayed request.
                ...
                if port == self._proxy_port:
                    ...
                    )  # BSD doesn't seem to preserve port information
                    source_port = port
...
class _Responder(object):
    ...
    def send(self, packet, ip, port, relayed, **kwargs):
        ...
        if relayed:
            broadcast_source = (
                packet.extractIPOrNone(FIELD_CIADDR) in IP_UNSPECIFIED_FILTER
            )
        else:
            broadcast_source = ip in IP_UNSPECIFIED_FILTER
        (broadcast_changed, original_was_broadcast) = packet.setFlag(
            FLAGBIT_BROADCAST, broadcast_source
        )

        # Perform any necessary packet-specific address-changes
        if (
            not original_was_broadcast
        ...
        if (
            broadcast_changed
        ):  # Restore the broadcast bit, in case the packet needs to be used for something else
...
class _L2Responder(_Responder):
    ...
    _ethernet_id = (
        None  #: The source MAC and Ethernet payload-type (and qtags, if applicable).
    )
    ...
    def _send(self, packet, ip, port, source_port=0, **kwargs):
        ...
        """
        mac = (
            packet.response_mac and MAC(packet.response_mac)
        ) or packet.getHardwareAddress()
...
class _L2Responder_AF_PACKET(_L2Responder):
    ...
    def __init__(self, server_address, response_interface, qtags=None):
        ...
        self._socket = socket.socket(
            _AF_PACKET, socket.SOCK_RAW, socket.htons(_ETH_P_SNAP)
        )
...
class _L2Responder_pcap(_L2Responder):
    ...
    def __init__(self, server_address, response_interface, qtags=None):
        ...
        self._fd = pcap.pcap_open_live(
            response_interface.encode("utf-8"),
            ctypes.c_int(0),
            ctypes.c_int(0),
            ctypes.c_int(0),
            errbuf,

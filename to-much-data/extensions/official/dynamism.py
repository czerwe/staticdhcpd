...
try:
    ...
    _logger.warning(
        "scapy is unavailable; addresses added to pools cannot be automatically ARPed"
    )
    arping = None
else:
    if system == "FreeBSD":
    ...
    if not l2sock:
        _logger.warning(
            "scapy was unable to construct layer-2 socketing rules; if using a BSD-based system (including OS X), please follow its documentation for installing pcap bindings; addresses added to pools cannot be automatically ARPed"
        )
        arping = None
    else:
        try:
            ...
            _logger.warning(
                "scapy's arping is unavailable; addresses added to pools cannot be automatically ARPed"
            )
            arping = None
        else:
            _logger.info("scapy imported successfully; automatic ARPing is available")

_LeaseDefinition = collections.namedtuple(
    "LeaseDefinition", ("ip", "mac", "expiration", "last_seen")
)
"""
...
class DynamicPool(object):
    def __init__(
        ...
        self._domain_name_servers = (
            domain_name_servers and [IPv4(i) for i in domain_name_servers] or None
        )
        ...
        self._logger.info(
            "Created dynamic provisioning pool '{}'".format(self._hostname_prefix)
        )
    ...
    def add_ips(self, ips, arp_addresses=True, arp_timeout=1.0):
        ...
        with self._lock:
            ...
            if arp_addresses and arping:
                ...
                mapped_ips_count = 0
                self._logger.info(
                    "Beginning ARP-lookup for {} IPs in pool '{}', with timeout={:.3f}s".format(
                        len(ips),
                        self._hostname_prefix,
                        arp_timeout,
                    )
                )
                (answered, unanswered) = arping(
                    ips.keys(), verbose=0, timeout=arp_timeout
                )
                for answer in answered:
                    try:
                        ...
                        self._logger.debug(
                            "Unable to use ARP-discovered binding {!r}: {}".format(
                                answer, e
                            )
                        )
                    else:
                        ...
                        self._logger.info(
                            "ARP-discovered {} bound to {} in pool '{}'; providing lease until {}".format(
                                ip_obj,
                                mac,
                                self._hostname_prefix,
                ...
                self._logger.info(
                    "{} IPs automatically bound in pool '{}'".format(
                        mapped_ips_count,
                        self._hostname_prefix,
                    )
                )
        ...
        self._logger.debug(
            "Added IPs to dynamic pool '{}': {}".format(
                self._hostname_prefix,
                sorted(ips.values()),
            )
        )
        self._logger.info(
            "Added {} available IPs to dynamic pool '{}'; new total: {}".format(
                len(ips),
                self._hostname_prefix,
                total,
            )
    ...
    def handle(self, method, packet, mac, client_ip):
        ...
        if method == "DISCOVER" or method.startswith("REQUEST:"):
            ...
            if definition and self._discourage_renewals:
                ...
                self._logger.debug(
                    "Setting T1 and T2 to 97.5% of lease-time={}: {} seconds".format(
        ...
        self._logger.info(
            "{} is unknown to the dynamic provisioning engine".format(method)
        )
        return None
    ...
    def get_leases(self, *args, **kwargs):
        ...
        with self._lock:
            for (mac, (expiration, ip)) in self._map.items():
                elements.append(
                    _LeaseDefinition(ip, mac, expiration, expiration - self._lease_time)
                )
        ...
        for lease in self.get_leases():
            writer.writerow(
                (
                    ...
                    lease.mac or "",
                    lease.expiration
                    and time.strftime(render_format, time.localtime(lease.expiration))
                    or "",
                    lease.last_seen
                    and time.strftime(render_format, time.localtime(lease.last_seen))
                    or "",
                )
            )
        output.seek(0)
    ...
    def show_leases_xhtml(self, *args, **kwargs):
        ...
        with self._lock:
            if not self._map:
                return "No leases yet assigned; {} IPs available".format(
                    len(self._pool)
                )

            elements = []
            for (mac, (expiration, ip)) in sorted(
                self._map.items(), key=(lambda element: element[1])
            ):
                elements.append(
                    """
                <tr>
    ...
    def _cleanup_leases(self):
        ...
        for (mac, ip) in dead_records:
            ...
            self._logger.debug(
                "Reclaimed expired IP {} from {} in pool '{}'".format(
                    ip,
                    mac,
                    self._hostname_prefix,
                )
    ...
    def _drop_lease(self, mac):
        ...
        if match:  # Drop the lease and reclaim the IP
            ...
            self._pool.append(ip)
            self._logger.info(
                "Reclaimed released IP {} from {} in pool '{}'".format(
                    ip,
                    mac,
                    self._hostname_prefix,
                )
    ...
    def _get_lease(self, mac, client_ip):
        ...
        if match:
            ...
            if client_ip and ip != client_ip:
                self._logger.info(
                    "Rejected request for {} from {} in pool '{}': does not match allocation of {}".format(
                        client_ip,
                        mac,
                        self._hostname_prefix,
                        ip,
            ...
            self._logger.info(
                "Extended lease of {} to {} in pool '{}' until {}".format(
                    ip,
                    mac,
                    self._hostname_prefix,
                    time.ctime(match[0]),
    ...
    def _allocate(self, mac, client_ip):
        ...
        if not ip:
            self._logger.error(
                "No IP available for assignment to {} in pool '{}'".format(
                    mac,
                    self._hostname_prefix,
                )
            )
    ...
    def _reclaim(self, mac, client_ip):
        ...
        if not ip:
            self._logger.warning(
                "No IP assigned to {} in pool '{}'".format(
                    mac,
                    self._hostname_prefix,
                )
            )
            ...
            self._logger.warning(
                "IP assigned to {}, {}, in pool '{}', does not match {}".format(
                    ip,
                    mac,
                    self._hostname_prefix,
                    client_ip,

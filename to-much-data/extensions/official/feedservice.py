...
_CONFIG = _config.extension_config_merge(
    defaults={
        ...
        "SERVER_BASE": "http://{}:{}".format(
            config.WEB_IP != "0.0.0.0" and config.WEB_IP or "localhost", config.WEB_PORT
        ),
...
_Event = collections.namedtuple(
    "Event", ("summary", "severity", "timestamp", "module", "line", "uid")
)
    ...
    def decorator(f):
        def function(*args, **kwargs):
            try:
                ...
                _logger.error(
                    "Unable to render {} feed:\n{}".format(
                        feed_type, traceback.format_exc()
                    )
                )
                raise
...
def _present_atom(logger):
    ...
    return (
        ...
        '<?xml version="1.0" encoding="utf-8"?>' + ET.tostring(feed),
    )
...
_LOGGER.setFormatter(
    logging.Formatter(
        """%(asctime)s : %(levelname)s : %(name)s:%(lineno)d[%(threadName)s]
<br/><br/>
%(message)s"""
    )
)
...
_logger.info(
    "Registering record-access-point at '{}'...".format(_CONFIG["PATH_RECORD"])
)
config.callbacks.webAddMethod(
    _CONFIG["PATH_RECORD"],
    _LOGGER.presentRecord,
...
if _CONFIG["PATH_ATOM"]:
    ...
    config.callbacks.webAddMethod(
        _CONFIG["PATH_ATOM"],
        lambda *args, **kwargs: _present_atom(_LOGGER),

...
def _parse_server_response(json_data):
    ...
    return Definition(
        ip=json_data["ip"],
        lease_time=json_data["lease_time"],
...
class _HTTPLogic(object):
    def __init__(self):
        ...
        self._default_name_servers = getattr(
            config, "X_HTTPDB_DEFAULT_NAME_SERVERS", ""
        )
    ...
    def _lookupMAC(self, mac):
        ...
        if self._post:
            ...
            request = urllib.request.Request(
                "{}?{}".format(
                    self._uri, urllib.parse.urlencode(parameters, doseq=True)
                ),
                headers=headers,
            )

        _logger.debug(
            "Sending request to '{}' for '{}'...".format(self._uri, parameters)
        )

        try:
            response = urllib.request.urlopen(request)
            _logger.debug(
                "MAC response received from '{}' for '{}'".format(self._uri, mac)
            )
...
class HTTPCachingDatabase(CachingDatabase, _HTTPLogic):
    def __init__(self):
        ...
        if hasattr(config, "X_HTTPDB_CONCURRENCY_LIMIT"):
            CachingDatabase.__init__(
                self, concurrency_limit=config.X_HTTPDB_CONCURRENCY_LIMIT
            )
        else:

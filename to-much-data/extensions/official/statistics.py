...
_CONFIG = _config.extension_config_merge(
    defaults={
...
_METHODS = tuple(
    sorted(getattr(dhcp, key) for key in dir(dhcp) if key.startswith("_PACKET_TYPE_"))
)

_logger = logging.getLogger("extension.statistics")

_Gram = collections.namedtuple(
    "Gram",
    (
        "dhcp_packets",
...
class Statistics(object):
    ...
    def __init__(self, graph_size, gram_size):
        ...
        self._graph = collections.deque(
            (None for i in range(graph_size)), maxlen=graph_size
        )
    ...
    def _update_graph(self):
        ...
        with self._lock:
            ...
            if self._gram_start_time <= current_time - self._gram_size:
                ...
                steps = int(
                    (current_time - self._gram_start_time) / max(1, self._gram_size)
                )
    ...
    def graph_csv(self):
        ...
        with self._lock:
            ...
            for (i, gram) in enumerate(reversed(self._graph)):
                record = [
                    time.strftime(
                        render_format, time.localtime(base_time - (i * self._gram_size))
                    )
                ]
                if gram:
    ...
    def graph(self, dimensions):
        ...
        with self._lock:
            ...
            for (i, gram) in enumerate(self._graph):
                gram_time = int(
                    (base_time - ((len(self._graph) - i - 1) * self._gram_size)) * 1000
                )

                if gram:
                    packets_per_second.append(
                        {
                            "x": gram_time,
                            "y": sum(gram.dhcp_packets.values()) / self._gram_size,
                        }
                    )
                    for method in _METHODS:
                        if method_values:
                            method_values[method].append(
                                {"x": gram_time, "y": gram.dhcp_packets[method]}
                            )
                        if method_discarded_values:
                            method_discarded_values[method].append(
                                {
                                    "x": gram_time,
                                    "y": gram.dhcp_packets_discarded[method],
                                }
                            )
                else:
        ...
        """ % {
            ...
            "show_legend": (method_values or method_discarded_values)
            and "true"
            or "false",
        }
    ...
    def lifetime_stats(self):
        ...
        received_total = 0
        received = [
            '<tr><td style="text-align: right; font-weight: bold;">Received</td>'
        ]
        processed = [
            '<tr><td style="text-align: right; font-weight: bold;">Processed</td>'
        ]
        discarded = [
            '<tr><td style="text-align: right; font-weight: bold;">Discarded</td>'
        ]
        with self._lock:
            ...
            <table class="element">
                ...
                methods="\n".join(
                    "<th>{}</th>".format(method.replace("REQUEST:", "R:"))
                    for method in _METHODS
                ),
                ...
                span=len(_METHODS) + 1,
                average=received_total
                and (self._processing_time / received_total)
                or 0.0,
                dhcp=received_total,
    ...
    def averages(self, windows):
        ...
        with self._lock:
            for window in windows:
                ...
                packets_discarded = sum(
                    self._current_gram["dhcp-packets-discarded"].values()
                )
                ...
                <tr>
                    ...
                    <td>{average:.4f}s</td>
                        ...
                        methods="\n".join(
                            "<td>{:.4f}/s</td>".format(packets[method] / total_time)
                            for method in _METHODS
                        ),
                        discarded=(packets_discarded / total_time),
                        other=(other / total_time),
                        average=(
                            total_packets and (processing_time / total_packets) or 0.0
                        ),
                    )
                )
            return """
...
if _CONFIG["GRAPH_ENABLED"]:
    ...
    _logger.info(
        "Registering graph as a dashboard element, with ordering={}".format(
            _CONFIG["GRAPH_ORDERING"]
        )
    )
    config.callbacks.webAddDashboard(
        _CONFIG["MODULE"],
        _CONFIG["GRAPH_NAME"],
        lambda path, queryargs, mimetype, data, headers: _stats.graph(
            _CONFIG["GRAPH_RENDER_DIMENSIONS"]
        ),
...
if _CONFIG["GRAPH_CSV_PATH"]:
    _logger.info(
        "Registering graph CSV-provider at '{}'".format(_CONFIG["GRAPH_CSV_PATH"])
    )
    config.callbacks.webAddMethod(
        _CONFIG["GRAPH_CSV_PATH"],
        lambda *args, **kwargs: _stats.graph_csv(),
...
if _CONFIG["GRAPH_JSON_PATH"]:
    _logger.info(
        "Registering graph JSON-provider at '{}'".format(_CONFIG["GRAPH_JSON_PATH"])
    )
    config.callbacks.webAddMethod(
        _CONFIG["GRAPH_JSON_PATH"],
        lambda *args, **kwargs: _stats.graph_json(),
...
if _CONFIG["AVERAGES_ENABLED"]:
    ...
    if _CONFIG["AVERAGES_DISPLAY"]:
        _logger.info(
            "Registering averages as a dashboard element, with ordering={}".format(
                _CONFIG["AVERAGES_ORDERING"]
            )
        )
        config.callbacks.webAddDashboard(
            _CONFIG["MODULE"],
            _CONFIG["AVERAGES_NAME"],
            renderer,
...
if _CONFIG["LIFETIME_STATS_ENABLED"]:
    ...
    if _CONFIG["LIFETIME_STATS_DISPLAY"]:
        _logger.info(
            "Registering lifetime stats as a dashboard element, with ordering={}".format(
                _CONFIG["LIFETIME_STATS_ORDERING"]
            )
        )
        config.callbacks.webAddDashboard(
            _CONFIG["MODULE"],
            _CONFIG["LIFETIME_STATS_NAME"],
            renderer,
            ordering=_CONFIG["LIFETIME_STATS_ORDERING"],
        )
    else:
        _logger.info(
            "Registering lifetime stats at '{}'".format(_CONFIG["LIFETIME_STATS_PATH"])
        )
        config.callbacks.webAddMethod(
            _CONFIG["LIFETIME_STATS_PATH"],
            renderer,

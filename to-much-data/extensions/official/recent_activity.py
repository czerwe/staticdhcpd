...
_Event = collections.namedtuple(
    "Event", ("time", "mac", "ip", "subnet", "serial", "method", "port")
)
...
def _drop_old_events():
    ...
    if dropped:
        _logger.debug(
            "Dropped {} events from recent activity due to age".format(dropped)
        )
...
_logger.info(
    "Prepared recent-activity-tracker for up to {} events, {} seconds old".format(
        _CONFIG["MAX_EVENTS"], _CONFIG["MAX_AGE"]
    )
)

if _CONFIG["CLEAR_ON_REINIT"]:
    _logger.info(
        "Registering callback handler to clear activity on reinitialisation..."
    )
    config.callbacks.systemAddReinitHandler(_flush)

if _CONFIG["DISPLAY_IN_DASHBOARD"]:
    _logger.info(
        "Registering activity-tracker as a dashboard element, with ordering={}".format(
            _CONFIG["DASHBOARD_ORDERING"]
        )
    )

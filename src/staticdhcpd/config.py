...
for conf_path in conf_search_paths:
    ...
    try:  # Attempt to import conf.py from the path
        conf = SourceFileLoader(
            "conf", os.path.join(conf_path, "conf.py")
        ).load_module()
        # print(repr(conf))
        # print(dir(conf))
    except IOError:
    ...
    raise ImportError(
        "Unable to find a suitable instance of conf.py; searched: {!r}".format(
            conf_search_paths
        )
    )
...
for key in [
    k for k in dir(conf) if k.isupper()
...
if hasattr(conf, "loadDHCPPacket"):
    if inspect.getargspec(conf.loadDHCPPacket).args == [
        "packet",
        "mac",
        "client_ip",
        "relay_ip",
        ...
        __PXEOptions = collections.namedtuple(
            "PXEOptions",
            (
                "client_system",
                "client_ndi",
        ...
        def loadDHCPPacket(
            packet, method, mac, definition, relay_ip, port, source_packet
        ):
            vendor_class = None
            ...
            if source_packet.isOption("vendor_class"):
                vendor_class = tuple(
                    sorted(
                        source_packet.getOption("vendor_class", convert=True).items()
                    )
                )
            if source_packet.isOption("vendor_specific"):
                vendor_specific = tuple(
...
class _Namespace(object):
    ...
    def extension_config_iter(self):
        ...
        for key in [
            k
            for k in dir(self)
            if not k.startswith("_") and not k.startswith("extension_config_")
        ]:  # Copy everything that looks useful

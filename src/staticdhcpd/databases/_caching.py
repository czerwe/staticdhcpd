...
class _DatabaseCache(Database):
    ...
    def lookupMAC(self, mac):
        ...
        if not definition:
            ...
            _logger.debug(
                "Found a match for '{}' in database-cache '{}'".format(_mac, self)
            )
    ...
    def cacheMAC(self, mac, definition, chained=False):
        _logger.debug(
            "Setting definition for '{}' in database-cache '{}'...".format(mac, self)
        )
        with self._cache_lock:
        ...
        if cache:
            ...
            for data in cache:
                ...
                definitions.append(
                    Definition(
                        ip=ip,
                        lease_time=details[6],
                        subnet=subnet_id[0],
                        serial=subnet_id[1],
                        hostname=hostname,
                        gateways=details[0],
                        subnet_mask=details[1],
                        broadcast_address=details[2],
                        domain_name=details[3],
                        domain_name_servers=details[4],
                        ntp_servers=details[5],
                        extra=extra,
                    )
                )
            if definitions:
    ...
    def _cacheMAC(self, mac, definition, chained):
        ...
        for definition in definitions:
            ...
            self._subnet_cache[subnet_id] = (
                definition.gateways,
                definition.subnet_mask,
                definition.broadcast_address,
...
class MemcachedCache(_DatabaseCache):
    ...
    def __init__(
        self, name, memcached_server_data, memcached_age_time, chained_cache=None
    ):
        """
        Initialises the cache.
    ...
    def _cacheMAC(self, mac, definition, chained):
        ...
        for definition in definitions:
            ...
            mac_list.append(
                (
                    definition.ip and str(definition.ip),
                    definition.hostname,
                    definition.extra,
                    subnet_id,
                )
            ...
            cache_records[self._create_subnet_key(subnet_id)] = json.dumps(
                (
                    definition.gateways
                    and ",".join(str(i) for i in definition.gateways),
                    definition.subnet_mask and str(definition.subnet_mask),
                    definition.broadcast_address and str(definition.broadcast_address),
                    definition.domain_name,
                    definition.domain_name_servers
                    and ",".join(str(i) for i in definition.domain_name_servers),
                    definition.ntp_servers
                    and ",".join(str(i) for i in definition.ntp_servers),
                    definition.lease_time,
                )
            )
                ...
                if result:
                    definitions.append(
                        Definition(
                            ...
                            broadcast_address=result[2],
                            domain_name=result[3],
                            domain_name_servers=result[4],
                            ntp_servers=result[5],
                            lease_time=result[6],
                            subnet=subnet,
                            serial=serial,
                            extra=extra,

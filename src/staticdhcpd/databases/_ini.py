...
class _Config(configparser.RawConfigParser):
    ...
    def getboolean(self, section, option, default):
        ...
        """
        return bool(
            str(self.get(section, option, default)).lower().strip()
            in (
                "y",
                "yes",
                "t",
...
class INI(Database):
        ...
        for section in reader.sections():
            ...
            if m:
                self._process_subnet(
                    reader, section, m.group("subnet"), int(m.group("serial"))
                )
            else:
                try:
                    mac = MAC(section)
    ...
    def _validate_references(self):
        ...
        for (mac, (_, _, subnet, _)) in self._maps.items():
            if subnet not in self._subnets:
                raise ValueError(
                    "MAC '{}' references unknown subnet '{}|{}'".format(
                        MAC(mac), subnet[0], subnet[1]
                    )
                )
    ...
    def lookupMAC(self, mac):
        ...
        if extra_map and extra_subnet:
            ...
            extra = (extra_map and extra_map.copy()) or (
                extra_subnet and extra_subnet.copy()
            )
        ...
        return Definition(
            ...
            broadcast_address=subnet[3],
            domain_name=subnet[6],
            domain_name_servers=subnet[5],
            ntp_servers=subnet[4],
            extra=extra,
        )

...
class Definition(object):
    ...
    broadcast_address = (
        None  #: The :class:`IPv4 <IPv4>` broadcast address to advertise (may be None)
    )
    ...
    def __init__(
        ...
        if not self.ip:
            raise ValueError(
                "An IP address is required for assignment; received: {!r}".format(ip)
            )
...
class CachingDatabase(Database):
    ...
    _resource_lock = (
        None  #: A lock used to prevent the database from being overwhelmed.
    )
    ...
    def __init__(self, concurrency_limit=2147483647):
        ...
        """
        _logger.debug(
            "Initialising database with a maximum of {} concurrent connections".format(
                concurrency_limit
            )
        )
        ...
        if config.USE_CACHE:
            ...
            if config.CACHING_MODEL == "in-process":
                if config.DISK_CACHE_PERSISTENT or config.DISK_CACHE:
                    try:
                        disk_cache = _caching.DiskCache(
                            config.DISK_CACHE_PERSISTENT and "persistent" or "disk",
                            config.DISK_CACHE_PERSISTENT,
                        )
                        if config.DISK_CACHE:
                            _logger.debug(
                                "Combining local caching database and persistent caching database"
                            )
                            self._cache = disk_cache
                        else:
                            _logger.debug(
                                "Setting up memory-cache on top of persistent caching database"
                            )
                            self._cache = _caching.MemoryCache(
                                "memory", chained_cache=disk_cache
                            )
                    except Exception:
                        _logger.error(
                            "Unable to initialise disk-based caching:\n{}".format(
                        ...
                        if config.DISK_CACHE_PERSISTENT and not config.DISK_CACHE:
                            ...
                            _logger.warning(
                                "Caching is disabled: memory-caching was not requested, so no fallback exists"
                            )
                else:
                ...
                self._cache = _caching.MemcachedCache(
                    "memcached",
            ...
            if self._cache:
                _logger.info(
                    "Database caching enabled; top-level cache: {}".format(self._cache)
                )
            else:
                _logger.warning(
                    "'{}' database caching could not be enabled".format(

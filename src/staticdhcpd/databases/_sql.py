...
class _DB20Broker(_SQLDatabase):
    ...
    _connection_details = (
        None  #: The module-specific details needed to connect to a database
    )
        ...
        try:
            ...
            if result:
                ...
                return Definition(
                    ...
                    broadcast_address=result[4],
                    domain_name=result[5],
                    domain_name_servers=result[6],
                    ntp_servers=result[7],
                    lease_time=result[8],
                    subnet=result[9],
                    serial=result[10],
...
class _PoolingBroker(_DB20Broker):
    ...
    def __init__(self, concurrency_limit):
        ...
        if config.USE_POOL:
            ...
            try:
                ...
                _logger.warning(
                    "eventlet is not available; falling back to unpooled mode"
                )
                return
            else:
    ...
    def _getConnection(self):
        if self._pool is not None:
            return self._eventlet__db_pool.PooledConnectionWrapper(
                self._pool.get(), self._pool
            )
        else:
...
class MySQL(_PoolingBroker):
    ...
    def __init__(self):
        ...
        _logger.debug(
            "MySQL configured; connection-details: {}".format(self._connection_details)
        )
...
class PostgreSQL(_PoolingBroker):
    ...
    def __init__(self):
        ...
        _logger.debug(
            "PostgreSQL configured; connection-details: {}".format(
                self._connection_details
            )
        )
...
class Oracle(_PoolingBroker):
    ...
    def __init__(self):
        ...
        _logger.debug(
            "Oracle configured; connection-details: {}".format(self._connection_details)
        )
...
class SQLite(_NonPoolingBroker):
    ...
    def __init__(self):
        ...
        _logger.debug(
            "SQLite configured; connection-details: {}".format(self._connection_details)
        )

    ...
    if ifaddr.ifa_addr:
        ...
        if sockaddr.sa_family == socket.AF_INET:  # IPv4 address
            sockaddr_in = ctypes.cast(
                ctypes.pointer(sockaddr), ctypes.POINTER(struct_sockaddr_in)
            ).contents
...
def _evaluate_mac(ifaddr, iface):
    if ifaddr.ifa_name.decode("utf-8") == iface:
        ...
        sockaddr_dl = ctypes.cast(
            ctypes.pointer(sockaddr), ctypes.POINTER(struct_sockaddr_dl)
        ).contents
...
def _extract_mac(ifaddr):
    ...
    if sockaddr.sa_family == _AF_PACKET:
        sockaddr_ll = ctypes.cast(
            ctypes.pointer(sockaddr), ctypes.POINTER(struct_sockaddr_ll)
        ).contents
        ...
        mac = sockaddr_dl.sdl_data[
            sockaddr_dl.sdl_nlen : sockaddr_dl.sdl_nlen + sockaddr_dl.sdl_alen
        ]

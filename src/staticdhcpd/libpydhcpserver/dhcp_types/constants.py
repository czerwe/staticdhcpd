...
TYPE_LONG_PLUS = (
    "32-bits+"  #: At least one multiple of four bytes in network-byte-order.
)
...
MAGIC_COOKIE_ARRAY = array.array(
    "B", MAGIC_COOKIE
...
DHCP_FIELDS_SPECS = {
    TYPE_IPV4: (4, 0, 1),
    TYPE_IPV4_PLUS: (0, 4, 4),
    TYPE_IPV4_MULT: (0, 0, 4),
    TYPE_BYTE: (1, 0, 1),
    TYPE_BYTE_PLUS: (0, 1, 1),
    TYPE_STRING: (0, 0, 1),
    TYPE_BOOL: (1, 0, 1),
    TYPE_INT: (2, 0, 1),
    TYPE_INT_PLUS: (0, 2, 2),
    TYPE_LONG: (4, 0, 1),
    TYPE_LONG_PLUS: (0, 4, 4),
...
DHCP_OPTIONS_TYPES = collections.defaultdict(
    lambda: _DHCP_OPTION_TYPE_UNASSIGNED,
    {
        0: TYPE_PAD,
        1: TYPE_IPV4,
        2: TYPE_LONG,
...
DHCP_OPTIONS_REVERSE = dict(
    (v, k) for (k, v) in DHCP_OPTIONS.items()

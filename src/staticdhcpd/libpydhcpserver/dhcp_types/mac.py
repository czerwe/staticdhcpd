...
class MAC(object):
    ...
    def __init__(self, address):
        ...
        if isinstance(address, int):
            if not 0 <= address <= 281474976710655:
                raise ValueError(
                    "'{ip}' is not a valid IP: not a 32-bit unsigned integer".format(
                        ip=address,
                    )
                )
            ...
            if isinstance(address, str):
                ...
                if len(address) != 12:
                    raise ValueError(
                        "Expected twelve hex digits as a MAC identifier; received {}".format(
                            len(address)
                        )
                    )
                ...
                if len(self._mac) != 6 or any(
                    (type(d) is not int or d < 0 or d > 255) for d in self._mac
                ):
                    raise ValueError(
                        "Expected a sequence of six bytes as a MAC identifier; received {!r}".format(
                            self._mac
                        )
                    )
    ...
    def __str__(self):
        if self._mac_string is None:
            self._mac_string = "{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}".format(
                *self._mac
            )

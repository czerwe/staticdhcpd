...
from .rfc import RFC, rfc3046_decode, rfc3925_decode, rfc3925_125_decode
...
class DHCPPacket(object):
    ...
    _maximum_size = (
        None  #: The maximum number of bytes permitted in the encoded packet.
    )
    ...
    word_size = (
        4  #: The number of bytes in a word; 32-bit by network convention by default.
    )
    ...
    def __init__(self, data=None, _copy_data=None):
        ...
        if (
            options_position != _PACKET_HEADER_SIZE
        ):  # Insert the cookie without padding.
            self._header[
                _MAGIC_COOKIE_POSITION:_PACKET_HEADER_SIZE
            ] = MAGIC_COOKIE_ARRAY
    ...
    def _locateOptions(self, data):
        ...
        if position == -1:
            raise ValueError(
                "Data received does not represent a DHCP packet: Magic Cookie not found"
            )
    ...
    def encodePacket(self):
        ...
        option_ordering = [
            i for i in _OPTION_ORDERING if i in keys
        ]  # Put specific options first
        option_ordering.extend(
            sorted(keys.difference(option_ordering))
        )  # Then sort the rest

        # Prepare the main payload
        size_limit = (
            (self._maximum_size or 0xFFFF) - _PACKET_HEADER_SIZE - 68 - 3
        )  # Leave some for the protocol header and three for option 52, if needed
        (payload, option_ordering) = self._packOptions(
            options, option_ordering, size_limit
        )
        ...
        for (field, option_52_value) in ((FIELD_SNAME, 2), (FIELD_FILE, 1)):
            if not option_ordering:  # There are no more options to allocate
                break
            if any(
                i for i in self.getOption(field) if i != "\0"
        ...
        if option_52:
            packet[
                -(4 + terminal_pad_size)
            ] = 52  # Option ID (takes the place of former END)
    ...
    def _serialiseOptionValue(self, option, value):
        ...
        """
        type = DHCP_FIELDS_TYPES.get(option) or DHCP_OPTIONS_TYPES.get(
            self._getOptionID(option)
        )
        if not type or not type in _FORMAT_CONVERSION_SERIAL:
            raise ValueError(
                "Requested option does not have a type-mapping for conversion: {option}".format(
                    option=value,
                )
            )
    ...
    def _deserialiseOptionValue(self, option, value):
        ...
        if not type in _FORMAT_CONVERSION_DESERIAL:
            raise ValueError(
                "Requested option does not have a type-mapping for conversion: {option!r}".format(
                    option=value,
                )
            )
    ...
    def _extractList(self, value, option=None):
        ...
        if not isinstance(value, list) or not self._validateByteList(value):
            ...
            raise TypeError(
                "Value supplied cannot be converted into a list of bytes: {value!r}".format(
    ...
    def _getOptionID(self, option):
        ...
        if id is None:
            raise LookupError(
                "Option {option!r} is unknown".format(
                    option=option,
                )
            )
        return id
    ...
    def setOption(self, option, value, validate=True, force_selection=False):
        ...
        if option in DHCP_FIELDS:
            ...
            if len(value) < length and option in DHCP_FIELDS_TEXT:
                ...
                raise ValueError(
                    "Expected a value of length {length}, not {value_length}: {value!r}".format(
                        length=length,
            ...
            if dhcp_field_specs:  # It's a normal option
                if validate:  # Validate the length of the value
                    ...
                    if fixed_length != length:
                        if length < minimum_length or length % multiple:
                            raise ValueError(
                                "Expected a value a multiple of length {length}, not {value_length}: {value!r}".format(
                ...
                raise LookupError(
                    "Unsupported option: {option}".format(
                        option=option,
                    )
                )
    ...
    def setSelectedOptions(self, added=None, removed=None):
        ...
        if added is None and removed is None:
            ...
            if added:
                self._selected_options.update(
                    i
                    for i in (self._getOptionID(option) for option in added)
                    if i is not None
                )
            if removed:
                self._selected_options.difference_update(
                    i
                    for i in (self._getOptionID(option) for option in removed)
                    if i is not None
                )
    ...
    def __str__(self):
        ...
        for (option_id, data) in sorted(self._options.items()):
            ...
            output.append(
                (
                    represent
                    and "\t[{selected}][{id:03d}] {name}: {result!r}"
                    or "\t[-][{id:03d}] {name}: {result}"
                ).format(

    ...
    def __init__(self, address):
        ...
        if isinstance(address, int):
            ...
            try:
                ...
                raise ValueError(
                    "{ip!r} is not a valid IPv4: non-integer data supplied".format(
                        ip=address,
                    )
                )
            else:
                if len(octets) < 4:
                    raise ValueError(
                        "{ip} is not a valid IPv4: length < 4".format(
                            ip=address,
                        )
                    )
                ...
                if any(True for i in octets if i < 0 or i > 255):
                    raise ValueError(
                        "{ip} is not a valid IPv4: non-byte values present".format(
                            ip=address,
                        )
                    )
    ...
    def isSubnetMember(self, address, prefix):
        ...
        if isinstance(prefix, int):
            if 0 <= prefix <= 32:
                ...
                raise ValueError(
                    "Invalid CIDR prefix: {prefix}".format(
                        prefix=prefix,
                    )
                )
        else:

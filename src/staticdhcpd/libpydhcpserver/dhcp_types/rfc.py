...
class rfc3361_120(RFC):
    def __init__(self, data):
        ...
        if ip_4_mode == dns_mode:
            raise ValueError(
                "'{data} contains both IPv4 and DNS-based entries".format(
                    data=data,
                )
            )
...
class rfc3442_121(RFC):
    def __init__(self, classless_addresses):
        ...
        for ((ip, mask), router) in classless_addresses:
            ...
            if not 0 <= mask <= 32:
                raise ValueError(
                    "CIDR mask {mask} is not between 0 and 32".format(
                        mask=mask,
                    )
                )
            width = mask // 8
...
class rfc5678_139(RFC):
    def __init__(self, values):
        ...
        for (code, addresses) in values:
            ...
            for token in [
                tok
                for tok in [address.strip() for address in addresses.split(",")]
                if tok
            ]:
                self._value.extend(IPv4(token))

...
class _PacketWrapper(object):
    ...
    source_address = (
        None  #: The :class:`libpydhcpserver.dhcp.Address` of the packet's origin.
    )
    ...
    def __exit__(self, type, value, tb):
        ...
        try:
            ...
            if self._discarded:
                _logger.debug(
                    "Discarded packet of type {} from {}".format(
                        self._packet_type, self.mac
                    )
                )

            time_taken = time.time() - self._start_time
            _logger.debug(
                "{} request from {} processed in {:.4f} seconds".format(
                    self._packet_type, self.mac, time_taken
                )
            )
    ...
    def _evaluateSource(self):
        ...
        if self.giaddr:  # Relayed request.
            if not config.ALLOW_DHCP_RELAYS:  # Ignore it.
                ...
                config.ALLOWED_DHCP_RELAYS
                and not self.giaddr in config.ALLOWED_DHCP_RELAYS
            ):
    ...
    def announcePacket(self, ip=None, verbosity=logging.INFO):
        ...
        _logger.log(
            verbosity,
            "{type} from {mac}{ip}{sip} received on port {port}".format(
                type=self._packet_type,
                mac=self.mac,
                ip=(ip and (" for {}".format(ip)) or ""),
                sip=(
                    self.source_address.ip
                    not in libpydhcpserver.dhcp.IP_UNSPECIFIED_FILTER
                    and " via {}:{}".format(
                        self.source_address.ip, self.source_address.port
                    )
                    or ""
                ),
                port=self.port,
    ...
    def loadDHCPPacket(self, definition, inform=False):
        ...
        if not process:
            _logger.info(
                "Ignoring {} from {} per loadDHCPPacket()".format(
                    self._packet_type, self.mac
                )
            )
        return process
    ...
    def retrieveDefinition(self, override_ip=False, override_ip_value=None):
        ...
        definition = self._server.getDatabase().lookupMAC(
            self.mac
        ) or config.handleUnknownMAC(
            self.packet,
            self._packet_type,
            self.mac,
            ip,
        ...
        if definition and not isinstance(definition, Definition):
            _logger.debug(
                "Multiple (count={}) definitions found".format(len(definition))
            )
...
def _dhcpHandler(packet_type):
    ...
    def decorator(f):
        def wrappedHandler(self, packet, source_address, port):
            with _PacketWrapper(
                self, packet, packet_type, source_address, port
            ) as wrapper:
...
class _DHCPServer(DHCPServer):
    ...
    def _handleDHCPDecline(self, wrapper):
        ...
        if wrapper.sid == self._server_address:  # Rejected!
            ...
            if definition and definition.ip == wrapper.ip:  # Known client.
                ...
                _logger.warning(
                    "{type} from {mac} for {ip}, but its assigned IP is {aip}".format(
                        type=wrapper.getType(),
                        mac=wrapper.mac,
                        ip=wrapper.ip,
                        aip=definition.ip,
                ...
                _logger.warning(
                    "{type} from {mac} for {ip}, but the MAC is unknown".format(
                        type=wrapper.getType(),
                        mac=wrapper.mac,
                        ip=wrapper.ip,
                    )
    ...
    def _handleDHCPDiscover(self, wrapper):
        ...
        if not wrapper.filterPacket(override_ip=True, override_ip_value=None):
            return
        wrapper.announcePacket()

        definition = wrapper.retrieveDefinition(
            override_ip=True, override_ip_value=None
        )
        if definition:
            if config.ENABLE_RAPIDCOMMIT and wrapper.packet.isOption(80):
                _logger.info(
                    "{} from {} requested rapid-commit".format(
                        wrapper.getType(), wrapper.mac
                    )
                )
    ...
    def _handleDHCPInform(self, wrapper):
        ...
        :except _PacketSourceBlocklist: The MAC can't be handled, so reduce
                                        load.
        """
        if not wrapper.filterPacket(override_ip=True, override_ip_value=wrapper.ciaddr):
            return
        ...
        definition = wrapper.retrieveDefinition(
            override_ip=True, override_ip_value=wrapper.ciaddr
        )
        if definition:
            wrapper.packet.transformToDHCPAckPacket()
            if wrapper.loadDHCPPacket(definition, inform=True):
                self._emitDHCPPacket(
                    ...
                    wrapper.port,
                    wrapper.mac,
                    wrapper.ciaddr or _IP_REJECTED,
                )
                wrapper.markAddressed()
        else:
    ...
    def _handleDHCPRequest(self, wrapper):
        ...
        if wrapper.sid and not wrapper.ciaddr:  # SELECTING
            ...
            not wrapper.sid and wrapper.ciaddr and not wrapper.ip
        ):  # RENEWING or REBINDING
            self._handleDHCPRequest_RENEW_REBIND(wrapper)
        else:
            _logger.warning(
                "{type} ({sid}|{ciaddr}|{ip}) from {mac} unhandled: packet not compliant with DHCP spec".format(
                    type=wrapper.getType(),
                    sid=wrapper.sid,
    ...
    def _handleDHCPRequest_RENEW_REBIND(self, wrapper):
        ...
        """
        renew = (
            wrapper.source_address.ip not in libpydhcpserver.dhcp.IP_UNSPECIFIED_FILTER
        )
        wrapper.setType(
            renew and _PACKET_TYPE_REQUEST_RENEW or _PACKET_TYPE_REQUEST_REBIND
        )
        if not wrapper.filterPacket():
            return
        wrapper.announcePacket(ip=wrapper.ip)
        ...
        if config.NAK_RENEWALS and (renew or config.AUTHORITATIVE):
            ...
            if definition and definition.ip == wrapper.ciaddr:
                ...
                if wrapper.loadDHCPPacket(definition):
                    self._emitDHCPPacket(
                        wrapper.packet,
                        libpydhcpserver.dhcp.Address(wrapper.ciaddr, 0),
                        wrapper.port,
                        wrapper.mac,
                        wrapper.ciaddr,
                    )
    ...
    def _handleDHCPRelease(self, wrapper):
        ...
        if wrapper.sid == self._server_address:  # Released!
            ...
            if definition and definition.ip == wrapper.ciaddr:  # Known client.
                ...
                _logger.warning(
                    "{} from {} for {}, but no assignment is known".format(
                        wrapper.getType(), wrapper.mac, wrapper.ciaddr
                    )
                )
            ...
            with self._lock:
                ...
                if not actions:
                    ...
                    if actions + 1 > config.SUSPEND_THRESHOLD:
                        _logger.warning(
                            "{} is issuing too many requests; ignoring for {} seconds".format(
                                mac, config.MISBEHAVING_CLIENT_TIMEOUT
                            )
                        )
                        self._ignored_addresses.append(
                            [minimal_mac, config.MISBEHAVING_CLIENT_TIMEOUT]
                        )
                        return False
        return True
    ...
    def _emitDHCPPacket(self, packet, address, port, mac, client_ip):
        ...
        _logger.info(
            "{type} sent at {mac} for {client} via {ip}:{port} [{bytes} bytes]".format(
    ...
    def addToTempBlocklist(self, mac, packet_type, reason):
        ...
        with self._lock:
            self._ignored_addresses.append(
                [tuple(mac), config.UNAUTHORIZED_CLIENT_TIMEOUT]
            )
        _logger.warning(
            "{mac} was temporarily blocklisted for {time} seconds following {packet_type}: {reason}".format(
                mac=mac,
    ...
    def evaluateAbuse(self, mac):
        ...
        with self._lock:
            ignored = [
                timeout
                for (ignored_mac, timeout) in self._ignored_addresses
                if mac == ignored_mac
            ]
        if ignored:
            raise _PacketSourceIgnored(
                "MAC is on cooldown for another {} seconds".format(max(ignored))
            )
...
class DHCPService(threading.Thread):
    ...
    def __init__(self, database):
        ...
        _logger.info(
            "Prepared to bind to {address}; ports: server: {server}, client: {client}, proxy: {proxy}{response_interface}".format(
                ...
                and "; raw-response-interface: {response_interface}{qtags}".format(
                    response_interface=config.DHCP_RESPONSE_INTERFACE,
                    qtags=config.DHCP_RESPONSE_INTERFACE_QTAGS
                    and "; raw-response-interface-qtags: {qtags!r}".format(
                        qtags=config.DHCP_RESPONSE_INTERFACE_QTAGS,
                    )
                    or "",
                )

...
def _parseAuthorization(authorization):
    ...
    """
    parameters = (
        p.strip() for p in authorization[authorization.find(" ") + 1 :].split(",")
    )
    result = {}
...
def _validateCredentials(parameters, method):
    ...
    try:
        ...
        raise ValueError(
            "Authorization data from client is not spec-compliant: {}".format(e)
        )
            ...
            try:
                ...
                if callback:
                    _validateRequest(
                        ...
                        callback.secure
                        or (
                            callback.display_mode == WEB_METHOD_DASHBOARD
                            and config.WEB_DASHBOARD_SECURE
                        ),
                    )
                    cacheable = callback.cacheable
                    if callback.display_mode == WEB_METHOD_DASHBOARD:
                        handler = lambda path, queryargs, mimetype, data, headers: _templates.renderDashboard(
                            path,
                            queryargs,
                            mimetype,
                            data,
                            headers,
                            featured_element=callback,
                        )
                    elif callback.display_mode == WEB_METHOD_TEMPLATE:
                        handler = lambda path, queryargs, mimetype, data, headers: _templates.renderTemplate(
                            path, queryargs, mimetype, data, headers, callback
                        )
                    else:
                ...
                (mimetype, data) = handler(
                    path, queryargs, mimetype, data, self.headers
                )
                ...
                _logger.debug(
                    "Authentication required to access {}: {}".format(
                        self.path, e.nonce
                    )
                )
                ...
                self.send_header(
                    "WWW-Authenticate",
                    "Digest {}".format(
                        ", ".join('{}="{}"'.format(key, value) for (key, value) in auth)
                    ),
                )
                self.end_headers()
            except Exception:
                ...
                _logger.error(
                    "Problem while processing request for '{}' via {}:\n{}".format(
                        self.path,
                        method,
                        error,
                    )
                ...
                _logger.debug(
                    "Processed {} from {}:{} for {} in {:.4f} seconds".format(
                        method,
                        self.client_address[0],
                        self.client_address[1],
                        self.path,
...
class WebService(threading.Thread):
    ...
    def __init__(self):
        ...
        self._web_server = _ThreadedServer(
            (config.WEB_IP, config.WEB_PORT), _WebHandler
        )

...
_WebDashboardElement = collections.namedtuple(
    "WebDashboardElement", ("ordering", "module", "name", "callback")
)
"""
...
"""
_WebMethod = collections.namedtuple(
    "WebMethod",
    (
        "module",
        "name",
        ...
        for (i, element) in enumerate(_web_dashboard):
            ...
            if ordering is None:
                if _web_dashboard:
                    ...
                    ordering = 0
            element = _WebDashboardElement(
                ordering, functions.sanitise(module), functions.sanitise(name), callback
            )
...
def retrieveVisibleMethodCallbacks():
    ...
    with _web_lock:
        return tuple(
            sorted(
                (element, path)
                for (path, element) in _web_methods.items()

...
_BOOT_TIME = datetime.datetime.now().replace(
    microsecond=0
...
def _renderHeaders(path, queryargs, mimetype, data, headers):
    ...
    for callback in retrieveHeaderCallbacks():
        try:
            ...
            _logger.error(
                "Unable to execute header-element {!r}:\n{}".format(
...
def _renderMain(elements, path, queryargs, mimetype, data, headers):
    ...
    for element in elements:
        ...
        try:
            ...
            _logger.error(
                "Unable to render dashboard element '{}' '{}':\n{}".format(
                    element.module,
                    element.name,
...
def _renderMethods():
    ...
    for (element, path) in retrieveVisibleMethodCallbacks():
        ...
        output.append(
            '<a href="{}" style="color: inherit;"{}>{}</a><br/>'.format(
                path,
                element.confirm
                and " onclick=\"return confirm('&quot;{} | {}&quot; requested that you confirm your intent to proceed');\"".format(
                    element.module,
                    element.name,
                )
                or "",
...
def _renderTemplate(
    elements, path, queryargs, mimetype, data, headers, rewrite_location=False
):
    """
...
def renderDashboard(path, queryargs, mimetype, data, headers, featured_element=None):
    ...
    return _renderTemplate(
        ...
        data,
        headers,
        rewrite_location=bool(featured_element),
    )

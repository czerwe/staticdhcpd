...
class Logger(object):
    ...
    def __init__(self):
        ...
        _logger.info(
            "Web-accessible logging online; buffer-size={}".format(
                config.WEB_LOG_HISTORY
            )
        )
    ...
    def render(self, *args, **kwargs):
        ...
        """
        max_height = config.WEB_LOG_MAX_HEIGHT and "max-height:{}px;".format(
            config.WEB_LOG_MAX_HEIGHT
        )
        ...
        for (severity, line) in self._handler.readContents():
            output.append(
                '<span class="{}">{}</span>'.format(
                    _SEVERITY_MAP[severity],
                    functions.sanitise(line).replace("\n", "<br/>"),
                )
            )
        return "<div style='overflow-y:auto;{}'>{}</div>".format(
            max_height, "<br/>\n".join(output)
        )
